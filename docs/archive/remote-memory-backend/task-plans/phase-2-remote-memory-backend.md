...
- [ ] T102 [P] [Backend] Implement LanceDB-backed recall and memory mutation endpoints.
  - DoD: `POST /v1/recall/generic`, `POST /v1/recall/reflection`, `POST /v1/memories/store`, `POST /v1/memories/update`, `POST /v1/memories/delete`, `POST /v1/memories/list`, and `POST /v1/memories/stats` exist with schema-accurate responses.
- [ ] T103 [P] [Backend] Implement SQLite-backed reflection job enqueue/status paths.
  - DoD: `POST /v1/reflection/jobs` and `GET /v1/reflection/jobs/{jobId}` persist and report job lifecycle states through SQLite with caller-scoped ownership enforcement.
...
- [ ] T105 [P] [QA] Add request-validation coverage for the newly frozen runtime semantics.
  - DoD: contract tests cover `tool-store` vs `auto-capture`, forbidden scope fields, frozen category enum validation, `nextOffset=null` terminal pagination, and caller-scoped stats/job visibility.
