...
- Completed tasks:
  ...
  - `T105`: Preserved backend-owned filtering and untrusted-data wrapping via orchestrator boundaries:
    - at phase-completion time, filtering still flowed through the existing backend path (`scopeManager.getAccessibleScopes(...)`),
    - tagged render path for generic recall still emitted explicit untrusted-data wrappers.
- Evidence commands:
