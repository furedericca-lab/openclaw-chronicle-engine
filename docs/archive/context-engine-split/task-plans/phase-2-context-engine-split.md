...
Tasks:
- [ ] T101 [Backend] Extract generic auto-recall planning into dedicated context/orchestration modules.
  - DoD: a new module owns generic recall row loading/filtering/selection composition; `index.ts` no longer contains the bulk of that inline orchestration logic.
- [ ] T102 [Backend] Extract reflection recall and error-signal prompt planning into dedicated modules.
  - DoD: reflection recall row planning and error reminder block preparation are delegated out of `index.ts`, with stable hook semantics preserved.
...
- [ ] T105 [Security] Preserve backend-authoritative filtering and untrusted-data wrapping in the extracted path.
  - DoD: code review/test evidence shows rendering cannot widen visibility beyond backend-owned filtering or downgrade current untrusted-data framing.
